...
const smsRowner = () => { return `\`\`\`¡¡ЭТА КОМАНДА МОЖЕТ ИСПОЛЬЗОВАТЬСЯ ТОЛЬКО МНОЙ КАК СОЗДАТЕЛЕМ БОТА!!\`\`\`` }//NUMÉRO DE BOT
const smsOwner = () => { return `\`\`\`¡¡ЭТУ КОМАНДУ МОЖЕТ ИСПОЛЬЗОВАТЬ ТОЛЬКО МОЙ СОЗДАТЕЛЬ\!!\`\`\`` }//OWNER
const smsMods = () => { return `\`\`\`¡¡Эту команду могут использовать только модераторы и мой создатель!!\`\`\`\`` }//MODÉRATEURS
const smsPremium = () => { return `\`\`\`¡¡ЭТА КОМАНДА ДОСТУПНА ТОЛЬКО ДЛЯ ПРЕМИУМ-ПОЛЬЗОВАТЕЛЕЙ И МОЕГО СОЗДАТЕЛЯ(ОВ) !! ЧТОБЫ ПОЛУЧИТЬ ПРЕМИУМ, КУПИТЕ ПРОЕЗДНОЙ, ИСПОЛЬЗУЯ #pass premium\`\`\`` }//UTILISATEURS PREMIUM
const smsGroup = () => { return `\`\`\`¡¡ЭТУ КОМАНДУ МОЖНО ИСПОЛЬЗОВАТЬ ТОЛЬКО В ГРУППАХ!!\`\`\`` }//PARA GRUPOS
const smsPrivate = () => { return `\`\`\`¡¡ЭТА КОМАНДА МОЖЕТ ИСПОЛЬЗОВАТЬСЯ ТОЛЬКО ЧАСТНЫМ ЛИЦОМ!!\`\`\`` }//AL PRIVADO
const smsAdmin = () => { return `\`\`\`¡¡ЭТА КОМАНДА ПРЕДНАЗНАЧЕНА ТОЛЬКО ДЛЯ АДМИНИСТРАТОРОВ!!\`\`\`` }//ADMINS
const smsBotAdmin = () => { return `\`\`\`¡¡ЭТА КОМАНДА ПРЕДНАЗНАЧЕНА ТОЛЬКО ДЛЯ АДМИНИСТРАТОРОВ!!\`\`\`` }//BOT CON ADMIN
const smsUnreg = () => { return `\`\`\`¡¡DEBE ESTAR REGISTRADO PARA USAR ESTE COMANDO, ESCRIBA #verificar PARA REGISTRARSE!!\`\`\`` }//VÉRIFIER
const smsRestrict = () => { return `\`\`\`¡¡ESTE ORDEN ESTÁ RESTRINGIDO POR MI CREADOR!!\`\`\`` }//COMMANDE RESTREINTE
